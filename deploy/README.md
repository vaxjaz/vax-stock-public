...
> to legacy factor rankings or stock scores. After the report is durably
> stored, `vaxstock-eod.service` sends it as `[新研究EOD] <trade_date>` with
> `claude.md` and `payload.json` attached. Delivery is idempotent per market
> trade date; state is kept in the gitignored
> `var/strategy/research_eod_mail_state.json`. Failed delivery is not marked
> sent and remains retryable.
