...
10. **EOD Prediction 线(实施中,MR-Eval E4) = zz800 seed → 用户 universe 自我迭代**:
   ...
   **任务拆解(后续 PR 顺序)**:
   ...
   - **E4-5 Prediction Layer2(下一步)**:`research/prediction_eval.py` join predictions/results,按 `action`/`direction`/`confidence_bucket`/`regime|macro_regime`/`concept` 分桶;`generation_mode=live/replay` 分开展示;不按样本数屏蔽,N 直接展示。
